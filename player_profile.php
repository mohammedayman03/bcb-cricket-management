...
if (isset($_GET['email'])) {
    ...
    if ($result) {
        ...
        if (isset($_POST['modify'])) {
            ...
            $checkEmailQuery = "SELECT * FROM player WHERE email = '$modifiedEmail' AND email != '$email'";
            ...
            if (mysqli_num_rows($checkEmailResult) > 0) {
                ...
                if ($updateResult) {
                    echo '<script type="text/javascript">
                            ...
                            window.location.href = "player_home.php?email=' . $modifiedEmail . '";
                          </script>';
                } else {
...
<body>
    ...
    <form action="" method="POST">
        ...
        <label>Type:</label>
        <select name="modified_type" required>
            <option value="Batsman" <?= ($playerData['type'] == 'Batsman') ? 'selected' : '' ?>>Batsman</option>
            <option value="Bowler" <?= ($playerData['type'] == 'Bowler') ? 'selected' : '' ?>>Bowler</option>
            <option value="Wicket Keeper" <?= ($playerData['type'] == 'Wicket Keeper') ? 'selected' : '' ?>>Wicket Keeper</option>
            <option value="All rounder" <?= ($playerData['type'] == 'All rounder') ? 'selected' : '' ?>>All rounder</option>
        </select><br>
    ...
    <p>Type: <?= $playerData['type'] ?></p>

    <br>


    <a href="player_home.php?email=<?= $playerData['email'] ?>">Go Back to Home</a>
