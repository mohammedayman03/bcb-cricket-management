...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_series_query = "SELECT * FROM twenty_twenty WHERE series_name = '$series_name' and email ='$player_email' ";
    $series_result = $con->query($check_series_query);

    if ($series_result->num_rows > 0) {
        $update_query = "UPDATE twenty_twenty SET wickets_taken = wickets_taken + $wickets_taken, runs_scored = runs_scored + $runs_scored
                         WHERE series_name = '$series_name' AND email = '$player_email'";
    } else {
        $update_query = "INSERT INTO twenty_twenty (email, name, series_name, wickets_taken, runs_scored)
